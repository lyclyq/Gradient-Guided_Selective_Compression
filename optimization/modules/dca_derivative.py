...
def main():
    ...
    # =========================
    dca_drv = None
    if (DCADerivative is not None) and has_arch and args.dca_derivative_enable:
        dca_drv = DCADerivative(DCADrvCfg(
            ...
            # 注意：有符号/强度的归一化参数使用 dca_derivative.py 的默认值（tanh/gain=1/clip=3）
        ))
            ...
            if has_arch and args.quickval_every > 0 and (global_step + 1) % args.quickval_every == 0:
                ...
                with torch.enable_grad():
                    ...
                    for xv, yv in va:
                        ...
                        # ====== DCA Derivative：用 L_alpha 作为 drift proxy（叠加权重可选） ======
                        if dca_drv is not None:
                            ...
                            drv = dca_drv.update_and_weight(proxy)
                            weight = drv["weight"]
                            u_gate = drv["u_gate"]
                            d_est  = drv["d_est"]

                            # 若不是 "只用趋势门控"，则对 L_alpha 做 (1 + λ_drv * weight) 放缩
                            if not args.dca_drv_only:
                        ...
                        # ====== 趋势门控：有符号×强度 推 resmix.beta（纯控制） ======
                        if (dca_drv is not None) and args.resmix_enable and hasattr(net, "blocks"):
                            try:
                                _proxy = float(L_alpha.detach().item())
                                drv2 = dca_drv.update_and_weight(_proxy)
                                # thrust = 方向 * 强度 ∈ [-1,1]
                                thrust = drv2["s_signed"] * drv2["m_mag"]
                                step = args.resmix_from_dca_lr * thrust
                                if step != 0.0:
                                    for b in net.blocks:
                        ...
                        # 可选：根据 DCA 分数微调 resmix beta（默认关闭）
