...
class SpectrumEMA(nn.Module):
    ...
    def update_1d(self, x_bt: torch.Tensor):
        # x_bt: [B,T,C]
        x_ct = x_bt.transpose(1,2)  # [B,C,T]
        spec = torch.fft.rfft(x_ct, dim=-1)
        mag = (spec.real**2 + spec.imag**2).sqrt().mean(dim=(0,1))  # [T/2+1]
    ...
    def update_2d(self, x_bchw: torch.Tensor):
        ...
        mag = (spec.real**2 + spec.imag**2).sqrt().mean(dim=(0,1,2))  # [W/2+1]
        self.spec = mag.clone() if self.spec is None else self.spec.mul(self.beta).add((1.0-self.beta)*mag)
...
def _mask_from_overlap(train_mag: torch.Tensor, val_mag: torch.Tensor, gamma: float, a_min: float):
    """Return attenuation mask A in [a_min,1] based on spectral overlap.
    Uses Hellinger-like distance between normalized spectra.
    """
    eps = 1e-8
    p = train_mag / (train_mag.sum() + eps)
    q = val_mag   / (val_mag.sum()   + eps)
    s = (p.sqrt() - q.sqrt()).abs()          # [F]
    A = torch.exp(-gamma * s)                # [F]
...
class _BaseMasker(nn.Module):
    def __init__(self, gamma: float = 1.0, a_min: float = 0.8, apply_on: str = "input", use_drv_gate: bool = False):
        ...
        # Derivative-driven gate (affects gamma)
        self.register_buffer("u_gate", torch.tensor(1.0))
        # External global gate (affects attenuation strength directly)
        self.register_buffer("external_gate", torch.tensor(1.0))
        self.spec_ema = SpectrumEMA(beta=0.95)

    @torch.no_grad()
    def set_gate(self, u: float):
        """Backward-compat: derivative gate in [0,1]; scales gamma if use_drv_gate=True."""
        self.u_gate.fill_(float(u))

    @torch.no_grad()
    def set_external_gate(self, g: float):
        """External soft gate in [0,1]; 1 = full attenuation, 0 = bypass (no extra attenuation).
        Implemented as A' = 1 - g * (1 - A) to preserve A's lower bound semantics.
        """
        self.external_gate.fill_(float(g))

    def _scale_gamma(self):
        return self.gamma * (0.5 + float(self.u_gate.item())) if self.use_drv_gate else self.gamma

    def _apply_external_gate(self, A: torch.Tensor) -> torch.Tensor:
        # A in [a_min,1]; external_gate in [0,1]; blend towards 1 (no attenuation) when gate small
        g = float(self.external_gate.item())
        if g >= 1.0:
            return A
        if g <= 0.0:
...
class FeatureFFTMaskWrapper1D(_BaseMasker):
    ...
    def forward(self, x_bt: torch.Tensor) -> torch.Tensor:
        ...
        x_ct = x_bt.transpose(1,2)                          # [B,C,T]
        spec = torch.fft.rfft(x_ct, dim=-1)
        mag = (spec.real**2 + spec.imag**2).sqrt().mean(dim=(0,1))  # [F]
        train_mag = self.spec_ema.get()
        if train_mag is None:
            return self.base(x_bt)
        A = _mask_from_overlap(train_mag.to(mag.device), mag, gamma=self._scale_gamma(), a_min=self.a_min)
        A = self._apply_external_gate(A)
...
class FeatureFFTMaskWrapper2D(_BaseMasker):
    ...
    def forward(self, x_bchw: torch.Tensor) -> torch.Tensor:
        ...
        spec = torch.fft.rfftn(x_bchw, dim=(-2,-1))
        mag = (spec.real**2 + spec.imag**2).sqrt().mean(dim=(0,1,2))  # [F]
        train_mag = self.spec_ema.get()
        if train_mag is None:
            return self.base(x_bchw)
        A = _mask_from_overlap(train_mag.to(mag.device), mag, gamma=self._scale_gamma(), a_min=self.a_min)
