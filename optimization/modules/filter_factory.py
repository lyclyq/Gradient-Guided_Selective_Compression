...
def make_filter_1d(channels: int,
    ...
    """Factory for 1D filters.
    - backend="lp_fixed" builds a fixed, non-trainable low-pass.
    - backend="darts" builds a DARTS-style gate if available.
    - feature_mask=True wraps with an FFT-based feature masker (denoise-only).
    """
    ...
    if feature_mask:
        base = FeatureFFTMaskWrapper1D(base_module=base,
                                       gamma=ff_gamma,
                                       a_min=ff_amin,
                                       apply_on=ff_apply_on,
                                       use_drv_gate=ff_use_drv_gate)
    return base
...
def make_filter_2d(channels: int,
    ...
    if feature_mask:
        base = FeatureFFTMaskWrapper2D(base_module=base,
                                       gamma=ff_gamma,
                                       a_min=ff_amin,
                                       apply_on=ff_apply_on,
                                       use_drv_gate=ff_use_drv_gate)
